...
This module implements a cascade of diffusion modules according to the paper
"Cascaded Diffusion Models for High Fidelity Image Generation" (https://arxiv.org/abs/2106.15282).

In particular, we combine a base stage DDPM model at 8x8 resolution with a
SR3 super-resolution model which goes from 8x8 -> 32x32.
"""
...
class GaussianDiffusion_DDPM(torch.nn.Module):
    ...
    def loss_on_batch(
        self, images, low_resolution_images: Optional[torch.Tensor], y=None
    ) -> Dict:
...
class GaussianDiffusion_CDM(torch.nn.Module):
    def __init__(self, config: DotConfig):
        super().__init__()
        self._config = config

        # Create the base stage diffusion model
        self._base_stage = GaussianDiffusion_DDPM(MNistUnet, config.base_stage)

        # Create the upsampling resolutions
        self._super_resolution_stages = torch.nn.ModuleList()
        for idx in range(config.num_cascades):
            stage_config = config[f"cascaded_stage_{idx}"]
            self._super_resolution_stages.append(
                GaussianDiffusion_DDPM(MNistUnet, stage_config)
            )
        self._is_class_conditional = self._super_resolution_stages[
            -1
        ]._is_class_conditional
    ...
    def sample(
        ...
        """Unconditionally/conditionally sample from the diffusion model."""
        all_stage_output = []
        # First sample from the base stage
        x_0 = self._base_stage.sample(
            low_res_context=None,
            num_samples=num_samples,
            guidance_fn=guidance_fn,
            classes=classes,
        )
        if self._base_stage._is_class_conditional:
            x_0 = x_0[0]

        all_stage_output.append(x_0)
        x_0 = normalize_to_neg_one_to_one(x_0)

        # Now sample from each cascade
        for sr_module in self._super_resolution_stages:
            x_0 = sr_module.sample(
                low_res_context=x_0,
                num_samples=num_samples,
                guidance_fn=guidance_fn,
                classes=classes,
            )
            if sr_module._is_class_conditional:
                x_0 = x_0[0]
            all_stage_output.append(x_0)
            x_0 = normalize_to_neg_one_to_one(x_0)

        return unnormalize_to_zero_to_one(x_0), all_stage_output

    def configure_optimizers(self, learning_rate):
        optimizers = []
        base_stage_optimizer = Adam(
            self._base_stage.parameters(), lr=learning_rate, betas=(0.9, 0.99)
        )
        optimizers.append(base_stage_optimizer)

        # Configure each of the cascade stage optimizers
        for sr_module in self._super_resolution_stages:
            optimizers.append(
                Adam(sr_module.parameters(), lr=learning_rate, betas=(0.9, 0.99))
            )
        return optimizers
