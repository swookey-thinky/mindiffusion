...
Trains the diffusion prior network from DaLL*E 2.
"""
...
OUTPUT_NAME = "output/prior"
os.environ["TOKENIZERS_PARALLELISM"] = "false"


def run_lesson_17_prior(
    ...
    validation_dataloader = DataLoader(
        validation_dataset, batch_size=batch_size, shuffle=True, num_workers=4
    )
    ...
    diffusion_model = GaussianDiffusion_DaLLE2_Prior(config=config)
    summary(
...
def sample(
    diffusion_model: GaussianDiffusion_DaLLE2_Prior,
    step,
    config: DotConfig,
    num_samples=64,
):
...
def save(diffusion_model, step, loss, optimizer):
    ...
    torch.save(
        ...
        f"{OUTPUT_NAME}/dalle2-prior-{step}.pt",
    )
