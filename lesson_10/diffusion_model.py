...
class GaussianDiffusion_SR3(torch.nn.Module):
    ...
    def loss_on_batch(self, images, low_resolution_images, y=None) -> Dict:
        ...
        model_output = self._score_network(
            torch.cat([x_t, low_res_x_0], dim=1), t=t, y=y
        )
        if self._is_v_param:
    ...
    def _p_mean_variance(
        ...
        model_output = (
            self._score_network(torch.cat([x, low_res_context], dim=1), t=t, y=y)
    ...
    def _p_sample_loop(self, shape, low_res_context, guidance_fn=None, classes=None):
    ...
    def _p_sample(self, x, t, low_res_context, y, guidance_fn=None):
