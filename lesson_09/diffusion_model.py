...
This contains several improvements over the original DDPM paper.

1.) v-param prediction model for learning an estimate of the variance of the reverse
    process model.
2.) Cosine beta schedule
3.) Importance sampling for the loss term, in v-param mode.
...
class GaussianDiffusion_ImprovedDDPM(torch.nn.Module):
    """Core Improved DDPM algorithm.

    Implements both DDPM and Improved DDPM training and sampling algorithms.
    """
    ...
    def loss_on_batch(self, image) -> Dict:
        ...
        x_t = self._q_sample(x_0=x0, t=t, noise=epsilon)
        ...
        model_output = self._score_network(x_t, t)
        if self._is_v_param:
    ...
    def sample(self, image_size: int, num_channels: int, batch_size: int = 16):
        """Unconditionally sample from the diffusion model."""
        # The output shape of the data.
        shape = (batch_size, num_channels, image_size, image_size)
        return self._p_sample_loop(shape)
    ...
    def _p_mean_variance(
        ...
        model_output = (
            self._score_network(x, t) if epsilon_v_param is None else epsilon_v_param
        )
        if self._is_v_param:
    ...
    def _p_sample_loop(self, shape):
        ...
        for t in tqdm(
            ...
            x_t_minus_1 = self._p_sample(x_t, t)
            x_t = x_t_minus_1
    ...
    def _p_sample(self, x, t: int):
        ...
        model_mean, _, model_log_variance = self._p_mean_variance(
            x=x, t=batched_times, clip_denoised=True
        )
    ...
    def _vb_bits_per_dim(
        ...
        mean, _, log_variance = self._p_mean_variance(
            x_t, t, clip_denoised=clip_denoised, epsilon_v_param=epsilon_v_param
        )
