...
class GaussianDiffusion_LDM(torch.nn.Module):
    """Core LDM algorithm.

    Defines the core LDM diffusion algorithm for training and sampling. This implements
    Algorithms 1 and 2 from the DDPM paper, using latent diffusion models.
    """

    def __init__(self, unet_type: Type, vae):
        super().__init__()
    ...
    def algorithm1_train_on_batch(self, image, context):
        ...
        # In this case, the inputs to the model are the latents generated from
        # the encoder of the VAE.
        x0 = self._vae.encode(normalize_to_neg_one_to_one(image))
    ...
    def algorithm2_sampling(
        ...
        # The input shape of the data. Note that the input to the model
        # is the latents dimension, NOT the dimension of the final images.
        shape = (batch_size, num_channels, latent_dim, latent_dim)
        ...
        # Convert from latent space to image space
        x_t = self._vae.decode(x_t)
