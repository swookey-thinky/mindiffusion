...
Trains the diffusion decoder network for DaLL*E 2 on MNIST.
"""
...
OUTPUT_NAME = "output/decoder"
os.environ["TOKENIZERS_PARALLELISM"] = "false"


def run_lesson_17_decoder(
    ...
    diffusion_model = GaussianDiffusion_DaLLE2_Decoder(config=config.diffusion_decoder)
    summary(
    ...
    diffusion_model, clip_embedder, optimizer = accelerator.prepare(
        diffusion_model, clip_embedder, optimizer
    )
    ...
    with tqdm(initial=step, total=num_training_steps) as progress_bar:
        ...
        while step < num_training_steps:
            ...
            loss_dict = diffusion_model.loss_on_batch(
                images, prompts=prompts, clip_embedder=clip_embedder
            )
            loss = loss_dict["loss"]
...
def sample(
    diffusion_model: GaussianDiffusion_DaLLE2_Decoder,
    step,
    config: DotConfig,
    validation_dataloader: DataLoader,
    clip_embedder: FrozenCLIPEmbedder,
    ...
    # Sample from the model to check the quality. We have to sample
    # conditionally.
...
def save(diffusion_model, step, loss, optimizer):
    ...
    torch.save(
        ...
        f"{OUTPUT_NAME}/dalle2-decoder-{step}.pt",
    )
...
def main(override=None):
    ...
    parser.add_argument("--config_path", type=str, default="configs/mnist_dalle2.yaml")

    args = parser.parse_args()

    run_lesson_17_decoder(
