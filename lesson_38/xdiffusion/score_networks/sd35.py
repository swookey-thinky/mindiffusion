...
from xdiffusion.layers.sd3 import AdaLayerNormContinuous, PatchEmbed
from xdiffusion.layers.sd35 import MMDitXBlock
from xdiffusion.utils import DotConfig


class SD35Transformer2DModel(torch.nn.Module):
    """
    The Transformer model introduced in Stable Diffusion 3.5. Includes an additional
    self attention channel on the image input, in addition to the cross attention layers.
    ...
    def __init__(
        ...
        self._config = config
        default_out_channels = self._config.input_channels
        self.out_channels = (
